      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            ...
            {pendingCnt > 0 && <span className="ml-2 text-amber-600 font-medium">· {pendingCnt} in attesa</span>}
          </p>
        </div>
        <div className="flex gap-2">
          {(['Pending', 'all'] as const).map(f => (
            ...
            >
              {f === 'Pending'
                ? <>In attesa {pendingCnt > 0 && <span className="ml-1 bg-white/30 px-1 rounded">{pendingCnt}</span>}</>
                : 'Tutte'
              }
            </button>
          ))}
        ...
        <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                {['Dipendente', 'Tipo', 'Dal', 'Al', 'Giorni', 'Motivo', 'Stato', ''].map(h => (
              ...
              {visible.length === 0 && (
                <tr>
                  <td colSpan={8} className="px-4 py-10 text-center text-gray-400">
                    {filter === 'Pending' ? 'Nessuna richiesta in attesa di approvazione.' : 'Nessuna richiesta.'}
                  </td>
                </tr>
              )}
              {visible.map(r => (
                <tr key={r.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-gray-600">{TYPE_LABEL[r.type] ?? r.type}</td>
                  <td className="px-4 py-3 text-gray-600 font-mono text-xs">{fmt(r.startDate)}</td>
                  <td className="px-4 py-3 text-gray-600 font-mono text-xs">{fmt(r.endDate)}</td>
                  <td className="px-4 py-3 text-gray-600">
                    {r.totalDays} {r.totalDays === 1 ? 'g.' : 'gg.'}
                  </td>
                  <td className="px-4 py-3 text-gray-500 max-w-[160px] truncate text-xs" title={r.reason}>
                    {r.reason || '—'}
                  </td>
                  ...
                  <td className="px-4 py-3 text-right space-x-3">
                    {r.status === 'Pending' && (
                      <>
                        <button
                          onClick={() => approve(r.id)} disabled={acting === r.id}
                          className="text-green-600 hover:text-green-800 text-xs font-medium disabled:opacity-50"
                        >
                          ✓ Approva
                        </button>
                        <button
                          onClick={() => reject(r.id)} disabled={acting === r.id}
